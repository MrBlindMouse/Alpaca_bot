...
class BarCache:
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS fetch_log (
                  ...
                  PRIMARY KEY (symbol, range_start, range_end)
                );
                """
            )
            cols = conn.execute("PRAGMA table_info(fetch_log)").fetchall()
    ...
    def upsert_bars(self, rows: List[BarRow]) -> int:
        if not rows:
            return 0
        with self._connect() as conn:
            conn.executemany(
                """
                INSERT INTO bars (symbol, ts, open, high, low, close, volume, vwap)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def mark_fetched(
        ...
        *,
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO fetch_log (symbol, range_start, range_end, timeframe, bar_count, fetched_at)
                VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(symbol, range_start, range_end) DO UPDATE SET
    ...
    def clear_fetch_log(self, symbol: str, range_start: str, range_end: str) -> None:
        with self._connect() as conn:
            conn.execute(
                """
    ...
    def clear_bars(self, symbol: str, range_start: str, range_end: str) -> None:
        with self._connect() as conn:
            conn.execute(
                """
                DELETE FROM bars
        ...
        with self._connect(readonly=True) as conn:
            rows = conn.execute(
                ...
                SELECT
                  ...
                  range_end,
                  CASE
                    WHEN MAX(CASE WHEN timeframe <> '' THEN 1 ELSE 0 END) = 1
                    THEN MAX(CASE WHEN timeframe <> '' THEN timeframe ELSE '' END)
                    ELSE ''
                  END AS timeframe,
    ...
    def list_timestamps(self, start: str, end: str) -> List[str]:
        with self._connect(readonly=True) as conn:
            rows = conn.execute(
                """
    ...
    def prices_at(self, ts: str, symbols: List[str]) -> Dict[str, float]:
        if not symbols:
            return {}
    ...
    def status(self) -> dict:
        with self._connect(readonly=True) as conn:
            bar_rows = conn.execute("SELECT COUNT(*), COUNT(DISTINCT symbol) FROM bars").fetchone()
